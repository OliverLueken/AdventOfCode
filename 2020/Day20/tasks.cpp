...
bool picture::insertTile(tile& t){
  if(field.empty()){
    ...
    field[origin]=t;
    maxx=0; minx=0; miny=0; maxy=0; length=1; height=1;
...
tile picture::picToTile()const{
  tile water;
  int tileLength = field.at({0,0}).data.at(0).size();
  int tileHeight = field.at({0,0}).data.size();
  for(int y=maxy; y>=miny; y--){
    for(int tiley=1; tiley<tileHeight-1; tiley++){
      std::string s;
...
bool isSnek(int x, int y, std::vector<size_t>& snekpos, tile& water){
...
int findSnakes(tile& water){
  ...
  int sneks=0;
  for(int y=0; y<water.data.size()-2; y++){
    for(int x=0; x<water.data[y].size()-19; x++){
      //std::cout << "findSnakes: " << x << ", " << y << std::endl;
      bool isSnake = isSnek(x,y,pos[0],water) && isSnek(x,y+1,pos[1],water) && isSnek(x,y+2,pos[2],water);
...
int countRoughWater(tile& water){
  int count=0;
  for(int y=0; y<water.data.size(); y++){
