...
std::map<std::string, std::string> getAllergenToIngredientMap(const strsetvec& ingredients, const std::vector<strvec>& allergens){
  std::map<std::string, std::set<std::string>> possibleIngredients;

  for(int i=0; i<allergens.size(); i++){
    for(auto& allergen:allergens.at(i)){
      if(possibleIngredients.find(allergen) == possibleIngredients.end()){
  ...
  do{
    ...
    for(auto it=possibleIngredients.begin(); it!=possibleIngredients.end(); ){
      if(it->second.size()==1){
    ...
    for(auto it=possibleIngredients.begin(); it!=possibleIngredients.end(); it++){
...
void countAllergentFreeIngredients(const strsetvec& ingredients, const std::map<std::string, std::string>& allergenToIngredient){
